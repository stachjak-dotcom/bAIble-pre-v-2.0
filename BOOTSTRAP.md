...
8. When a material project rule is not already defined, record the decision explicitly using [WORKSPACE_TEMPLATE/DECISIONS.md](WORKSPACE_TEMPLATE/DECISIONS.md). Do not turn an implementation choice into an authoritative project rule by implication.
9. Run one small, reversible experiment using [EXPERIMENTS.md](EXPERIMENTS.md).
10. Record verification separately from the agent's report using [VERIFICATION_RECORD.md](VERIFICATION_RECORD.md).
11. Transfer work using [HANDOFF.md](HANDOFF.md) when another human, agent, tool, or session takes over.
12. Run [BOOTSTRAP_TEST.md](BOOTSTRAP_TEST.md) to check whether the public contract is sufficient without hidden context.
13. Add orchestration only when coordination becomes useful.
14. Add contextual/relational knowledge only when it solves a real problem.
15. Add integration infrastructure when multiple tools, repositories, CI systems, or workflows need coordination.
16. Add a watchdog last. It should detect and report before it autonomously repairs.
17. Promote lessons into permanent rules only after validation. Use [STATUS_TRANSITIONS.md](STATUS_TRANSITIONS.md).
